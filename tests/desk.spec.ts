...
test('the <p> adapter round-trips a note body', async ({ page }) => {
	...
	await page.locator('.journal-row', { hasText: 'Round trip' }).getByText('edit', { exact: true }).click();
...
test('a soft break inside a paragraph survives as <br>', async ({ page }) => {
	...
	await page.locator('.journal-row', { hasText: 'Soft breaks' }).getByText('edit', { exact: true }).click();
...
test('unknown tags in a stored body are dropped gracefully on load', async ({ page }) => {
	await signIn(page);
	await nav(page, 'writing desk').click();
	await page.locator('.journal-row', { hasText: 'The home lab ate my weekend' }).getByText('edit', { exact: true }).click();
...
test('the note publish pill uses the lifecycle endpoint', async ({ page }) => {
	const mock = await signIn(page);
	await nav(page, 'writing desk').click();
	const row = page.locator('.journal-row', { hasText: 'The home lab ate my weekend' });
	await row.getByText('○ pencilled').click();
